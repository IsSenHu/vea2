  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType === 'edit'?'编辑角色':'新角色'">
      <el-form :model="role" label-width="80px" label-position="left">
        ...
        <el-form-item v-if="showGroup && !isGroup" label="角色组">
          <el-switch
            v-model="role.isGroup"
            style="display: block"
            active-color="#13ce66"
...
export default {
  ...
  methods: {
    checkIsGroup() {
      requestByClient(Auth, 'get', '/api/role/isGroup', null, resp => {
    ...
    getList() {
      ...
      requestByClient(Auth, 'post', '/api/role/page', this.listQuery, resp => {
        const respJson = resp.data
        const { code } = respJson
        if (code === 0) {
