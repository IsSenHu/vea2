      ...
      <el-form :model="blog" label-width="80px" label-position="left">
        ...
        <el-form-item label="上传博客">
          ...
          >
            ...
            <div slot="tip" class="el-upload__tip">只能上传markdown/html文件</div>
          </el-upload>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    async confirm() {
      ...
      if (isEdit) {
        requestByClient(Blog, 'post', '/api/blog/updateHtml', this.config, resp => {
          const respJson = resp.data
          const { code } = respJson
          if (code === 0) {
            this.showSuccess()
        ...
        requestByClient(Blog, 'put', '/api/blog/createHtml', this.config, resp => {
          const respJson = resp.data
          const { code } = respJson
          if (code === 0) {
            this.showSuccess()
    ...
    uploadFileMethod(param) {
      ...
      requestByClient(Blog, method, '/api/blog/' + api, formData, resp => {
        const respJson = resp.data
        const { code, data } = respJson
        if (code === 0 && data) {
          this.$message({
    ...
    sync(scope) {
      const url = scope.row ? '/api/blog/sync?isMarkDown=true&id=' + scope.row.id : '/api/blog/sync'
